/*
	Tools and scripts for Sharon Hargus' Deg Xinag website
*/
...
function BDAnnPage() {
	...
	this.end_annotate = function() {
		...
		this.f.action=window.location.protocol+"//depts.washington.edu/degxinag/annsaver/index.cgi";
		this.f.method="POST";
...
function ChapmanPage(){
	...
	src.setAttribute('src', "//depts.washington.edu/dxmedia/"+
										mediapath+"/wav/0.wav");
...
function AudioPage() {
	...
	this.loadTS = function(pagetype) {
		...
		xhr.open("GET", window.location.protocol+"//"+
										window.location.hostname+"/degxinag/timestamps/"+
				...
				if (xhr.status==200){
					...
					for(var idx in this.timestamps){
						if(this.timestamps[idx].length>0) {
							...
							else if(pagetype=="bd-ann") { // then idx can be sentence-starts
	...
	this.reformatWinIGT = function() {
		// find the spot where we're overflowing
		var ofidx = -1;
		var leftmarg = parseInt(window.innerWidth*.2)
		var pix = leftmarg; // start at the left margin of the win (20% of width)
		var winigt = this.getWinIGT();
		console.log("winigt children"+winigt.children.length);
		var wds = winigt.getElementsByClassName("word");
		var gls = winigt.getElementsByClassName("gloss");

		for(var i=0;i<wds.length;i++) {
			pix += wds[i].clientWidth;
			console.log(pix,this.win.clientWidth);
			if(pix>this.win.clientWidth+leftmarg){ ofidx=i; break; }
		}

		var firstWRow = winigt.getElementsByClassName("words")[0];
		var firstGRow = winigt.getElementsByClassName("glosses")[0];
		var transRow = winigt.getElementsByClassName("trans")[0];
		transRow.setAttribute("class","");
		var nextWRow = document.createElement("span");
		var nextGRow = document.createElement("span");
		var t = document.createElement("div");
		var u = document.createElement("div");
		nextWRow.setAttribute("class","words");
		nextGRow.setAttribute("class","glosses");

		// move these guys to the new row
		while(wds.length>ofidx){
			nextWRow.appendChild(wds[ofidx]);
			nextGRow.appendChild(gls[ofidx]);
		}
		t.style.display="table";
		t.style.left="50px";
		t.style.position="relative";
		u.style.display="table";
		t.appendChild(nextWRow);
		t.appendChild(nextGRow);
		u.appendChild(firstWRow);
		u.appendChild(firstGRow);
		winigt.appendChild(u);
		winigt.appendChild(t);
		winigt.appendChild(transRow);
		winigt.style.display="block";

		// don't know why i get random brs added ...
		// i can remove them tho
		var brs = winigt.getElementsByTagName("br");
		while(brs.length>0){
			console.log(brs[0]);
			winigt.removeChild(brs[0]);
		}

		console.log("winigt children"+winigt.children.length);
		console.log("ofidx",ofidx);
	}
